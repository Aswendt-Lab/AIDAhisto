...
def main():
    ...
    parser.add_argument('-w', '--width', default=width, help='kernel width (default %d)' % (width,))
    parser.add_argument('-c', '--channel', default=color_ch, help='color channel (default red with %i)' % (color_ch,),type=int)
    parser.add_argument('-m', '--min_dist', help='min. distance (default width/2)')
    parser.add_argument('-d', '--dark_peak_inv', action='store_false', help='dark peak inverter')
    parser.add_argument('-b', '--barFilterType', action='store_false', help='detect not circuar  square cellls')
